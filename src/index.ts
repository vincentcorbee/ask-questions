...
export const askQuestions = async (questions: Question[] = [], info: Info = {}) => {
  ...
  while (config) {
    const { question, allowed, name, required, value, show, default: defaultValue } = config;

    if (!show || (typeof show === 'function' && show(answers))) {
      const answer = await rl.question(`${styler.green('?')} ${styler.bold(question)}${allowed ? ` [${styler.cyan(allowed.join(','))}] ` : ' '}`);

      if (!answer && required) {
        rl.write(styler.brightRed(`⚠ ${question} is ${styler.bold('required')}.`));
        rl.write('\n');
